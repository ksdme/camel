...
vi.mock("@/main/utils/auth/refresh", () => ({ issueRefreshToken, verifyRefreshToken }));
vi.mock("@/main/utils/auth/tokens", () => ({ issueAccessToken }));
vi.mock("@/main/repos", () => ({
  getSessionRepo: () => sessionRepo,
}));

const { refresh } = await import("@/main/services/auth/refresh");
...
describe("POST /auth/refresh", () => {
  ...
  it("rejects requests with no refresh token", async () => {
    ...
    expect(result.json).toEqual({ code: "unauthenticated", message: "missing refresh token" });
  ...
  it("rejects invalid refresh tokens and records an audit failure", async () => {
    ...
    expect(result.statusCode).toBe(401);
    expect(result.json).toEqual({ code: "unauthenticated", message: "invalid refresh token" });
  ...
  it("rotates an active refresh token and sets new auth cookies", async () => {
    ...
    expect(sessionRepo.isActive).toHaveBeenCalledWith("old-jti", "user-1");
