...
import { mobileLoginTokens } from "../../services/auth/cache";
...
export const consumeMobileLoginToken = api.raw(
  ...
  async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      if (!cached) {
        sendJson(res, 401, { code: "unauthenticated", message: "mobile login token is invalid or expired" });
        return;
      }

      const deleted = await mobileLoginTokens.delete({ token: parsed.token });
      if (deleted === 0) {
        sendJson(res, 401, { code: "unauthenticated", message: "mobile login token has already been used" });
        return;
      }
