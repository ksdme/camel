...
// Verifies current password, hashes and stores the new one, then revokes
// every other refresh token for the user (the caller's current session is
// preserved so they don't get logged out of the tab they're using).
export const changePassword = api(
  ...
  async (req: ChangePasswordRequest): Promise<ChangePasswordResponse> => {
    ...
    // Find the caller's current refresh session by access-token jti → user.
    // We can't map access jti to a single refresh row, so we preserve the
    // most-recently-used active refresh token for the user (their current
    // browser) and revoke everything else.
